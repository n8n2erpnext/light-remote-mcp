...
await invoke({action:'enrollment-cancel',payload:{enrollmentId:'enr_12345678-1234-1234-1234-123456789abc'}},{'x-bridge-session':'bridge-session-test'});
...
const begin=calls.find(c=>c.path==='/operator/enrollments/begin');const cancel=calls.find(c=>c.path==='/operator/enrollments/cancel');const poll=calls.find(c=>c.path==='/operator/enrollments/poll');const hb=calls.find(c=>c.path.includes('/heartbeat'));const approve=calls.find(c=>c.path==='/operator/enrollments/approve');const revoke=calls.find(c=>c.path.includes('/revoke'));
...
if(cancel?.options.bridgeSession!=='bridge-session-test'||approve?.options.bridgeSession!=='bridge-session-test'||revoke?.options.bridgeSession!=='bridge-session-test')throw new Error('owner_bridge_session_not_forwarded');
