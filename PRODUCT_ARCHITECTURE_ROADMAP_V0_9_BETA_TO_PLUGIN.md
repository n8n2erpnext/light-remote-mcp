...
`ChatGPT Plus -> @Vercel -> api/operator?via=plus -> device-scoped Device Access Grant -> Vercel OIDC -> ARM Hub -> explicit ARM/AMD/Windows target`.

Before RDC disappears, a fresh Plus chat using only `@Vercel` must bootstrap the account devices, select one explicit connected device, attach to an existing Device Access Grant or create a ten-minute request that is approved on that device Local Wall, then pass durable Agent session open/resume/hold/close, encrypted exec/job/output, filesystem mutation, Git/build-test, process/network/service reads where supported, and policy-authorized maintenance actions. The returned `ps` is scoped to `grantId + deviceId + connectionId` and bounded by the finite Device Connection Lease; it is not a fixed one-hour human-approval token. Multiple ChatGPT windows may attach to the same active device grant without re-approval. The Plus bridge accepts GET at the ChatGPT-facing edge because the current Vercel connector fetch surface cannot attach arbitrary project headers or POST bodies. Production or Preview Vercel functions may relay calls only after exact Vercel OIDC verification at ARM. Mutations remain idempotent through stable operation IDs.
