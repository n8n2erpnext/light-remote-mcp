...
if(!html.includes('Device policy')||!html.includes('sudo-on-demand')||!html.includes('/api/devices/'))throw new Error('device_policy_page_contract_missing');
...
for(const marker of ["app.post('/operator/devices/:id/policy', softRateLimit, requireOperatorIdentity","wallApp.get('/device-policy', wallAuth.requirePage","wallApp.post('/api/devices/:id/policy', wallAuth.requireApi"])if(!server.includes(marker))throw new Error(`device_policy_auth_guard_missing:${marker}`);
