...
function toolHelperView(contextEnvelope={}){
  ...
  return {
    ...
    chooseTool:[
      ...
      'Use desktop-status/windows/frame/semantic-* only when the target advertises desktop capability. desktop-input additionally requires the separately owner-enabled desktop-input capability. Prefer semantic state for continuous reasoning and use frame as visual checkpoint/resync. Do not substitute Playwright unless the owner explicitly asks for browser automation.',
    ...
    tools:{
      ...
      desktop:{actions:['desktop-status','desktop-windows','desktop-frame','desktop-semantic-attach','desktop-semantic-snapshot','desktop-semantic-events','desktop-semantic-detach'],when:'Read the actual interactive desktop state on a target that advertises desktop capability.',capability:'desktop',payload:`${common.replace('...','limit?,screen?,maxWidth?,maxHeight?,quality?,scope?,maxDepth?,maxNodes?,semanticSessionId?,afterSeq?')}`,ops:{status:'Read interactive session, screens, cursor and foreground window.',windows:'List visible top-level windows; optional limit 1..200.',frame:'Capture one bounded JPEG frame from the real Windows desktop. Defaults max 960x540 quality 50; hard limits 1280x720 and quality 25..70.',semanticAttach:'Open a bounded Windows UI Automation semantic session; scope foreground|desktop, defaults foreground.',semanticSnapshot:'Refresh the bounded UIA tree for semanticSessionId and advance stateSeq.',semanticEvents:'Read the bounded/coalesced UIA event journal after afterSeq; gap or resyncRecommended means refresh semantic state/frame as appropriate.',semanticDetach:'Close semanticSessionId.'},rule:'The desktop view path is read-only. Semantic sessions use epoch/stateSeq, continuously journal UIA focus/structure/safe metadata changes, and do not read textbox or password contents. Prefer semantic-events for the live reasoning loop; use full snapshot/frame only for resync.'},
      desktopInput:{actions:['desktop-input'],when:'Control the real Windows mouse or keyboard only after the owner explicitly enables desktop-input on the target.',capabilities:['desktop','desktop-input'],payload:`${common.replace('...','events,semanticSessionId?,afterSeq?,settleMs?')}`,events:{move:'{type:"move",x,y}',click:'{type:"click",button?:"left"|"right"|"middle",count?:1..3,x?,y?}',wheel:'{type:"wheel",delta:-1200..1200,x?,y?}',text:'{type:"text",text}',key:'{type:"key",key,modifiers?:["CTRL"|"ALT"|"SHIFT"|"WIN"]}'},ack:'When semanticSessionId is supplied, the same call returns inputSeq, stateSeq, cursor, foreground, focused element, semantic events after afterSeq, gap/resync flags and hasMore. settleMs defaults to 90 and is bounded 0..250.',rule:'Mutation is locally denied by default. Validate semantic session before input, then use the returned closed-loop ACK for the next decision. Call semantic-events only for hasMore/late events or resync. Windows UIPI/secure-desktop boundaries remain in force.'},
