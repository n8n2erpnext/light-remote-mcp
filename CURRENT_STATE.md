...
- That path is the hosted/central Wall credential. Each **device Local Wall** has a separate config beside its device state (default `~/.config/gpt-operator-agent/wall-auth.json`), also `0600`, with its own cookie-signing secret. Non-loopback device Wall startup fails closed when this config is absent.
...
- Current owner operation is ChatGPT Plus, so write-capable direct custom MCP is not the continuity path. Until a reviewed Light Remote Plugin/App is available, Plus control traverses `@Vercel` -> `api/operator?via=plus` -> Vercel OIDC -> ARM Hub. The new public golden path is A-code-first: the owner opens one device Wall, gives its short-lived A code to the Agent, `connect(A)` consumes it and returns B, and B must be entered/approved on that exact device Wall. Approval binds that device into an opaque Agent client session; additional devices are paired independently into the same authorized set. Legacy `devices-bootstrap/authorize-*` remains compatibility/debug-only.
...
- Direct OAuth/MCP protocol dogfood is PASS and remains useful for eligible workspaces/future publication, but it is no longer the owner continuity gate. The release-blocking Plus gate is now `A -> connect -> B -> exact-device Wall Approve -> ready/client -> list only authorized devices -> durable per-device sessions/exec/job/output`, including two independently paired devices in one Agent client session. Agent lanes remain keyed by `(deviceId, agentId)` and use 15–60 minute HOLD/reconnect grace underneath each device grant.
...
- P4 live acceptance is now proven on AMD through ChatGPT Plus -> Vercel -> ARM: two Agent lanes ran under the approved device connection, Wall activity/history is live, Disconnect closed the grant/sessions while the service survived, and reconnect required fresh approval. Follow-up commit `93dfcd0` restored the missing Gateway proxy for device-scoped Wall activity.
- P4.5 is active in source: short-lived one-time **A code (device pairing/discovery) -> B code (Wall approval)**, compact `connect/connect-poll`, idempotent ready polling, and persistent Agent-client multi-device authorized bindings are implemented with regressions. Until P5 account registration/login lands, hosted and per-device Walls use the same transitional `operator` login credential for consistency, but each Wall keeps an independent session-signing secret; account login will replace this bootstrap credential without changing A/B pairing. A supplied A code goes straight to connect; A alone never grants execution. `list-devices` exposes only that client's separately approved bindings. Security review also added fail-closed device-local owner auth for every non-loopback Local Wall; public/private reverse-proxied Wall content, A issuance, `/approve`, APIs and SSE are not readable without that owner session. Live two-device acceptance is still pending, so P4.5 is not closed yet.
- P4.5 pre-deploy engineering gate on 2026-09-12 is 53/53 selftests PASS, root + Gateway audits report 0 known vulnerabilities, syntax/diff checks pass, and the new local-Wall auth regression covers unauthenticated page/API/SSE denial, cookie tamper rejection, CSRF/origin rejection, `0600` verifier storage and no plaintext password persistence.
...
## P5 account-plane live checkpoint — 2026-09-12

- Registered-owner Account Portal is now live at `https://lightremote.thaiduy.digital`. The production deployment for commit `ceaa2b3` is READY on Vercel with the function budget still locked at 12/12.
- Portal surfaces now include `/register/`, `/login/`, Devices landing, Add device, Revoke/Revoke all, Usage and Settings placeholders, and the ChatGPT/Claude/Any MCP client usage block.
- Durable account authority is hosted by the Light Remote Server/Hub, not Vercel filesystem state. Passwords use scrypt; browser account sessions use opaque tokens with only hashes persisted server-side; the browser receives a Secure/HttpOnly/SameSite=Strict cookie.
- First owner registration is single-owner migration only. Gateway requires the current transitional Wall password before registration and strips that secret before proxying to the Operator. Commit `ceaa2b3` additionally requires `ownerProofVerified=true` at the Operator socket boundary, so a caller cannot bypass the Gateway owner proof merely by reaching the local registration endpoint.
- Account state is still empty at this checkpoint. No owner account has been created yet.
- ARM Operator and Gateway are live with the account-plane source. Operator was rolled through its normal `Restart=always` lifecycle; Gateway was rebuilt from canonical source. Production Vercel runtime errors are zero after deployment.
- Wall login is intentionally NOT migrated yet. The transitional `operator` credential remains authoritative until the product owner creates the registered owner account successfully.
- Next human gate: product owner opens `/register/`, chooses the Light Remote account email/password, and supplies the Current Wall password as the one-time owner migration proof. After successful registration, migrate Hosted Wall and device Local Walls to registered account verification, then retire the transitional credential path after rollback-safe acceptance.
