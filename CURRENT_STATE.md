...
- Current owner operation is ChatGPT Plus, so write-capable direct custom MCP is not the continuity path. Until a reviewed Light Remote Plugin/App is available, Plus control traverses `@Vercel` -> `api/operator?via=plus` -> device-scoped Device Access Grant -> Vercel OIDC -> ARM Hub. A fresh chat first bootstraps devices and selects one explicit connected device. If that device has no active grant, its Local Wall receives a pending request and the enrolled service signs Approve/Deny; the hosted Wall cannot grant it. The returned `ps` capability is scoped to `grantId + deviceId + connectionId` and dies with the finite Device Connection Lease, not after a separate fixed one-hour human-approval TTL. Multiple ChatGPT windows may attach to the same active device grant without re-approval.
...
- Direct OAuth/MCP protocol dogfood is PASS and remains useful for eligible workspaces/future publication, but it is no longer the owner continuity gate. The release-blocking gate is a fresh ChatGPT Plus session using only `@Vercel`: `devices-bootstrap` -> explicit device -> reuse or Local-Wall approval of a Device Access Grant -> durable Agent session/exec/job/output -> routine ARM/AMD/Windows work without RDC. Agent lanes are keyed by `(deviceId, agentId)` and use 15–60 minute HOLD/reconnect grace underneath the parent device grant.
