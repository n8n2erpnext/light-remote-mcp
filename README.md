...
## Read endpoints
On the v0.6 development branch, Vercel read/operator proxy requests do not require a static shared Bearer secret. After session open, append `sid=<sessionId>&aid=<agentId>` so reads renew and audit the correct session:
- `GET /api/ping`
...
## Security
v0.6 removes the static shared Bearer secret from the development Vercel boundary. Vercel OIDC still authenticates Vercel -> ARM, and privileged payloads still use X25519 + HKDF-SHA256 + AES-256-GCM, short expiry, replay rejection and semantic idempotency. The eventual public App/Plugin must use explicit account/device authorization and ChatGPT permission/confirmation semantics rather than reintroducing a long-lived shared Bearer secret. Never place credentials/tokens/private keys/cookies in URL query strings.
...
`codex/v0.6-device-presence` separates device presence from operator-session lifetime, adds configurable per-session leases, and removes the static bridge Bearer requirement. It is branch-only and not production. See `DEVICE_PRESENCE_V0_6.md`.
