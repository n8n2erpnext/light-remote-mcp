...
function req(body){return new Promise((resolve,reject)=>{const b=Buffer.from(JSON.stringify(body));const q=http.request({socketPath:socket,path:'/v1/execute',method:'POST',headers:{'content-type':'application/json','content-length':b.length}},r=>{let d='';r.on('data',c=>d+=c);r.on('end',()=>resolve({status:r.statusCode,json:JSON.parse(d)}));});q.on('error',reject);q.end(b);});}
const operationId='idempotency-selftest-v03';
const payload={action:'exec_batch',operationId,cwd:'/tmp',script:`printf x >> ${marker}`,sessionId:'idempotency-test',note:'same logical operation',waitMs:5000,timeoutMs:10000};
