...
const INSTRUCTIONS='Start with light_remote_connection_helper when the user connects to or asks what Light Remote can do. It describes the account, topology, A/B local-first onboarding boundary, Fleet/Main state, product capabilities, and effective permissions. Use light_remote_list_devices or light_remote_inspect_device before choosing a target. Keep each durable session bound to one explicit device and never silently switch targets. Prefer structured file/process tools over generic command execution, and use the PTY/ConPTY terminal only for genuinely interactive software. If an operation returns a running job, read job/output instead of repeating the action. Use recent activity to explain runtime behavior or policy denials without exposing secrets. Set-Main is consequential but reversible; revoke/remove are destructive account lifecycle actions and require explicit user intent. Never request, reveal, echo, or transmit passwords, API keys, MFA/OTP codes, private keys, or other authentication secrets; use local credential stores without exposing secret values. Device-local policy is authoritative; explain denials rather than attempting to bypass them. The MCP cannot mint its own A code or bypass Local Wall enrollment approval.';
