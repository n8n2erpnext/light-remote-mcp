...
function toolHelperView(contextEnvelope={}){
  ...
  return {
    ...
    chooseTool:[
      ...
      'Use desktop-* only when the target advertises desktop capability. Real Remote controls the actual OS desktop; do not substitute DOM injection or Playwright for desktop work unless the owner explicitly asks for browser automation.',
    ...
    tools:{
      ...
      desktop:{actions:['desktop-status','desktop-windows'],when:'Read the actual interactive desktop state on a target that advertises desktop capability.',capability:'desktop',payload:`${common.replace('...','limit?')}`,ops:{status:'Read interactive session, screens, cursor and foreground window.',windows:'List visible top-level windows; optional limit 1..200.'},rule:'Real Remote V0.1 is read-only. These actions use the existing Light Remote Windows app hidden helper and Win32, not browser DOM automation.'},
