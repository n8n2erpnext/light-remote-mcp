...
- Current owner operation is ChatGPT Plus, so write-capable direct custom MCP is not the continuity path. Until a reviewed Light Remote Plugin/App is available, Plus control traverses `@Vercel` -> `api/operator?via=plus` -> owner-approved short-lived Plus session -> Vercel OIDC -> ARM Hub. The bridge is GET-only at the ChatGPT edge, but execution remains unavailable until the owner explicitly approves the pairing request in Wall; no anonymous execution or static shared bearer is reintroduced.
...
- Direct OAuth/MCP protocol dogfood is PASS and remains useful for eligible workspaces/future publication, but it is no longer the owner continuity gate. The release-blocking gate is a fresh ChatGPT Plus session using only `@Vercel`, performing the Wall pairing handshake and then proving device discovery, durable session/exec/job/output and routine ARM/AMD/Windows work without RDC.
