...
export class NativeSearchRegistry {
  ...
  view(searchId,owner=null){const row=this._row(searchId,owner);return {searchId:row.searchId,accountId:row.accountId,deviceId:row.deviceId,sessionId:row.sessionId,agentId:row.agentId,path:row.path,searchType:row.searchType,pattern:row.pattern,literalSearch:row.literalSearch,ignoreCase:row.ignoreCase,filePattern:row.filePattern,contextLines:row.contextLines,state:row.state,startedAt:row.startedAt,firstResultAt:row.firstResultAt,finishedAt:row.finishedAt,cancelledAt:row.cancelledAt,error:row.error,scannedFiles:row.scannedFiles,scannedDirs:row.scannedDirs,resultCount:row.results.length,maxResults:row.maxResults};}
}
