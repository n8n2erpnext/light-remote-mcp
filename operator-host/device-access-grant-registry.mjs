...
export class DeviceAccessGrantRegistry {
  constructor({stateFile=null,emit=()=>{},now=()=>Date.now()}={}){
    this.stateFile=stateFile;this.emit=emit;this.now=now;
  ...
  _load(){
    ...
    try{
      ...
      for(const row of Array.isArray(parsed.grants)?parsed.grants:[])if(row?.grantId)this.grants.set(String(row.grantId),row);
  ...
  request({accountId,deviceId,connectionId,connectionExpiresAt,agentId=null,label='ChatGPT Plus'}={}){
    ...
    if(active)return {state:'approved',grant:{...active},request:null,pollToken:null};
  ...
  approve(requestId,{deviceId=null,connectionId,connectionExpiresAt}={}){
    const id=validId(requestId,'invalid_plus_request_id'),row=this.requests.get(id),now=this.now();
    ...
    const grant=prior||{grantId:`dag_${crypto.randomUUID()}`,accountId:row.accountId,deviceId:row.deviceId,connectionId:cid,approvedAt:now,expiresAt,closedAt:null,closeReason:null};
  ...
  assert(grantId,{deviceId=null,connectionId=null}={}){
