...
for(const token of ['terminal','desktop','desktop-input','capability_refresh_device_identity_changed','windows-capability-refresh-same-identity=PASS','windows-capability-refresh-default-off=PASS'])assert.ok(windowsCapabilityRefreshAcceptance.includes(token),'Windows capability refresh acceptance missing: '+token);
