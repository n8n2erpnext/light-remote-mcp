...
- Direct OAuth/MCP protocol dogfood is PASS and remains useful for eligible workspaces/future publication, but it is no longer the owner continuity gate. The release-blocking gate is a fresh ChatGPT Plus session using only `@Vercel` through a protected preview bridge, with device discovery, durable session/exec/job/output and routine ARM/AMD/Windows work proven without RDC.
