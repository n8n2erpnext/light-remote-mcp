...
5. If the response is `pending`, retain `requestId + pollToken` privately, show the one-time code to the owner, and tell them to open the **Local Wall on the selected device** (default `http://127.0.0.1:5491/`). The hosted `/plus-authorize` page is informational only and cannot grant access.
6. The device Local Wall shows the pending ChatGPT request. The owner chooses **Approve** or **Deny** there. Device approval is Ed25519-signed by the enrolled device service. The authorization request expires after ten minutes.
...
Device lifetime is a separate authority. The always-alive local service may be running while the cloud is Dormant. A user-created Device Connection Lease opens cloud traffic for a finite hard window (reference caps: Free 4h, Pro 24h, VIP 72h); tool activity cannot silently extend that hard limit. If no Agent lane remains and no activity occurs for the configured 15–60 minute grace, the connection may close early. Hard expiry closes the device connection, its Device Access Grant and every Agent lane on that device.
...
## Wall / observability
There are two distinct control surfaces. **Local Wall** is served by the always-alive client service on the device (default `http://127.0.0.1:5491/`). It exists even before enrollment and while cloud state is Dormant. Local Wall owns Connect/Disconnect, reconnect-grace configuration, per-device Agent session visibility, and signed Approve/Deny for pending ChatGPT Device Access requests. It does not require a Terminal process to stay open.
