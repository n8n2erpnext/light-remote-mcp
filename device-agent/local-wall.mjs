...
<div class="card sessions"><div class="row" style="justify-content:space-between"><b>Pending ChatGPT access</b><span id="accessCount" class="muted small">0</span></div><div id="access" class="muted small">No pending access requests.</div></div>
...
function render(j){const s=j.local||{},remote=j.remote||null,enrolled=!!s.enrolled,connected=enrolled&&s.cloudState==='connected'&&s.cloudDesiredConnected;q('device').textContent=(s.deviceName||s.deviceId||'device')+(s.accountId?' · '+s.accountId:'');q('cloud').textContent=!enrolled?'Not enrolled':connected?'Connected':'Dormant';q('cloud').className='big '+(connected?'ok':'warn');q('lease').textContent=connected?[(s.connectionPlan||'plan').toUpperCase(),remaining(s.hardExpiresAt)].filter(Boolean).join(' · '):'No server channel';q('connect').disabled=busy||connected||!enrolled;q('disconnect').disabled=busy||!connected;if(s.reconnectGraceMs)q('grace').value=String(Math.round(s.reconnectGraceMs/60000));q('grace').disabled=busy||!connected;const access=remote?.access?.pending||[];q('accessCount').textContent=access.length+' request'+(access.length===1?'':'s');q('access').innerHTML=access.length?access.map(x=>'<div class="session"><b>'+esc(x.label||'ChatGPT')+'</b> · <span class="warn">'+esc(x.userCode||'pending')+'</span><br><span class="muted">agent '+esc(x.agentId||'')+' · expires '+new Date(x.expiresAt||Date.now()).toLocaleTimeString()+'</span><div class="row" style="margin-top:8px"><button class="btn primary" data-access-approve="'+esc(x.requestId)+'">Approve</button><button class="btn danger" data-access-deny="'+esc(x.requestId)+'">Deny</button></div></div>').join(''):'No pending access requests.';q('access').querySelectorAll('[data-access-approve]').forEach(b=>b.onclick=()=>accessAct(b.dataset.accessApprove,'approve'));q('access').querySelectorAll('[data-access-deny]').forEach(b=>b.onclick=()=>accessAct(b.dataset.accessDeny,'deny'));const rows=remote?.sessions||[];q('sessionCount').textContent=rows.length+' session'+(rows.length===1?'':'s');q('sessions').innerHTML=rows.length?rows.map(x=>'<div class="session"><b>'+esc(x.label||x.sessionId)+'</b> · '+esc(x.state)+'<br><span class="muted">agent '+esc(x.agentId||'')+' · jobs '+(x.activeJobs||[]).length+' · last '+new Date(x.lastSeenAt||Date.now()).toLocaleTimeString()+'</span></div>').join(''):'No active or held Agent sessions.';}
...
async function accessAct(id,kind){await act('/api/access/'+encodeURIComponent(id)+'/'+kind,{});}
...
export function startLocalWall({host='127.0.0.1',port=5491,brandSvgPath,getLocalStatus,getRemoteStatus,connect,disconnect,setGrace,accessApprove,accessDeny}={}){
  ...
  const server=http.createServer(async(req,res)=>{
    try{
      ...
      const access=url.pathname.match(/^\/api\/access\/(pa_[A-Za-z0-9_-]{20,80})\/(approve|deny)$/);
      if(req.method==='POST'&&access){const data=await body(req);const fn=access[2]==='approve'?accessApprove:accessDeny;if(typeof fn!=='function')return json(res,501,{ok:false,error:'access_control_unavailable'});const value=await fn(access[1],data||{});return json(res,200,{ok:true,authorization:value});}
