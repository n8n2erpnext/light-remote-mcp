...
for(const token of ['DesktopSessions','DesktopSessionLimit = 8','desktopSessionId','FrameSeq','ContentSeq','MinIntervalMs','OmitUnchanged','desktop_session_stale_topology','DesktopFrameRetryAfter','RecordDesktopFrame','DesktopFrameThrottled'])assert.ok(desktopSessionHelper.includes(token),`Windows visual desktop session contract missing: ${token}`);
...
assert.ok(agent.includes("minIntervalMs:Math.max(0,Math.min(Number.isFinite(Number(request.minIntervalMs))?Math.floor(Number(request.minIntervalMs)):250,5000))")&&agent.includes("omitUnchanged:request.omitUnchanged!==false"));
...
assert.ok(api.includes("desktop.minIntervalMs=Math.max(0,Math.min(Number.isFinite(Number(d.minIntervalMs))?Math.floor(Number(d.minIntervalMs)):250,5000))")&&api.includes("desktop.omitUnchanged=d.omitUnchanged!==false"));
assert.ok(toolHelper.includes("desktop-status")&&toolHelper.includes("desktop-attach")&&toolHelper.includes("desktop-resume")&&toolHelper.includes("desktop-detach")&&toolHelper.includes("desktop-windows")&&toolHelper.includes("desktop-frame")&&toolHelper.includes("minIntervalMs?")&&toolHelper.includes("omitUnchanged?")&&toolHelper.includes("desktopSessionId?")&&toolHelper.includes("contentSeq")&&toolHelper.includes("throttled=true")&&toolHelper.includes("retryAfterMs")&&toolHelper.includes("frameSha256")&&toolHelper.includes("desktop-semantic-attach")&&toolHelper.includes("desktop-semantic-snapshot")&&toolHelper.includes("desktop-semantic-events")&&toolHelper.includes("desktop-semantic-detach")&&toolHelper.includes("desktop-input")&&toolHelper.includes("provider?,scope?")&&toolHelper.includes("cdpEndpoint?,targetId?,urlMatch?")&&toolHelper.includes("browser-cdp")&&toolHelper.includes("loopback Chromium DevTools")&&toolHelper.includes("displayTopologyId?")&&toolHelper.includes("semanticSessionId?,afterSeq?,settleMs?")&&toolHelper.includes("toScreen?")&&toolHelper.includes('type:"drag"')&&toolHelper.includes("desktop_input_stale_topology")&&toolHelper.includes("inputMapping")&&toolHelper.includes("localX=round(frameX*xScale)")&&toolHelper.includes("windows-uia or browser-cdp")&&toolHelper.includes("inputSeq")&&toolHelper.includes("capabilities:['desktop','desktop-input']"));
