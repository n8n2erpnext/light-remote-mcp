...
Because RDC is approaching end-of-availability, beta promotion now additionally requires Light Remote self-dogfood. The candidate includes native owner OAuth for the MCP endpoint and a policy-bounded remote execution tool catalog. Local live proof passes OAuth PKCE/refresh plus ARM/AMD/Windows execution through the real operator socket. This does not lower the Vercel OIDC/bridge-session boundary. The follow-up dogfood adds typed text-file read/write, directory listing, literal search, process listing and process termination and proves those against ARM/AMD/Windows through the same encrypted durable-exec path. Static regression is 39/39 with zero root/gateway audit findings. Public endpoint acceptance is now PASS on `mcp.dashboard.thaiduy.store`: OAuth discovery, PKCE/replay/refresh, MCP tools, durable ARM exec and AMD/Windows fleet plus typed convenience operations all passed through HTTPS, followed by zero active sessions. Fresh-ChatGPT custom MCP App acceptance remains required before the beta tag.
