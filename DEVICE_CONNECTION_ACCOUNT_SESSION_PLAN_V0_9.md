...
P4.5 source checkpoint now includes A registry, A->B connect/poll, idempotent approved poll, Agent-client multi-device bindings, compact authorized-device listing/routing, and fail-closed non-loopback Local Wall owner authentication. It remains **in progress until live two-device acceptance is complete**. P4's current device-grant and session lifecycle remains the compatibility substrate while this pairing/client-session layer is implemented.
