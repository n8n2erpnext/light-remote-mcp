...
async function touchSession(ctx, tool, bridgeSession) {
  if (!ctx.tracked) return null;
  return callOperator(`/operator/sessions/${encodeURIComponent(ctx.sid)}/touch`, { method:'POST', body:{ agentId:ctx.aid, action:`read:${tool}` }, bridgeSession });
}
async function runReadTool(req, res, name, args = {}) {
  ...
  try {
    ...
    if (ctx.tracked) await touchSession(ctx, name, bridgeSession);
    const upstream = await callMcpTool(name, args, { bridgeSession });
