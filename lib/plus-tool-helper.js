...
function toolHelperView(contextEnvelope={}){
  ...
  return {
    ...
    chooseTool:[
      ...
      'Use process-* for interactive or long-running processes with incremental input/output.',
    ...
    tools:{
      ...
      process:{actions:['process-start','process-input','process-output','process-list','process-stop'],when:'Long-running or interactive commands.',startPayload:`${common.replace('...','script,cwd?,timeoutMs?,requiredCapabilities?')}`,note:'Use process-output offsets for incremental reads and process-input for stdin.'},
      exec:{action:'exec',when:'Bounded one-shot shell/PowerShell/zsh work not represented by a structured tool.',payload:`${common.replace('...','script,cwd?,timeoutMs?,waitMs?,requiredCapabilities?')}`,rule:'Prefer structured tools first; local platform policy re-infers capabilities before spawn.'},
      scp:{action:'scp',when:'Copy binary or large files between the Agent side and the exact target device.',payload:`${common.replace('...','scp')}`,ops:{uploadBegin:'{op:"upload-begin",destination,totalBytes,sha256,chunkBytes?,overwrite?,createParents?}',uploadChunk:'{op:"upload-chunk",transferId,index,data,sha256}',uploadCommit:'{op:"upload-commit",transferId}',downloadBegin:'{op:"download-begin",source,chunkBytes?}',downloadChunk:'{op:"download-chunk",transferId,index}',status:'{op:"status",transferId}',cancel:'{op:"cancel",transferId}'},encoding:'Chunk data is base64url on the wire; the runtime adapter should decode/encode bytes directly and never expose that encoding as user-visible content.',recipes:{chatAttachmentToDevice:['Read the user attachment as raw bytes from the chat/runtime file handle.','Compute whole-file sha256 and call upload-begin.','Send ordered upload-chunk calls with raw chunks encoded only at the transport boundary.','Call upload-commit and verify returned totalBytes/sha256.'],deviceFileToChat:['Call download-begin on the absolute source path.','Read download-chunk indexes until complete and verify each chunk plus whole-file sha256.','Materialize the reconstructed bytes as a chat attachment/preview using the host runtime file API.','Do not use exec, stdout, or text regeneration for the file.']},rules:['Use the transfer sha256 and per-chunk sha256 to verify integrity.','Default chunk size is 1 MiB; service bounds are enforced by the target.','SCP transfers are owner-bound to account/device/session/agent and require filesystem capability.','Never tunnel file bytes through exec stdout when scp is available.']},
    ...
    safety:['Always keep deviceId/sessionId/agentId from the current context aligned.','Never silently fall back to another device when the target is offline or denied.','Local device policy is the final capability boundary.','Pair each additional device independently with A/B approval.']
  };
}
