...
const resolveClientDevice=async({clientSessionId,agentId:aid,deviceId})=>{const b=clients.resolve(clientSessionId,{agentId:aid,deviceId:deviceId||null});const grant=access.assert(b.grantId,{deviceId:b.deviceId,connectionId:b.connectionId});return {binding:b,grant,device:devices.get(b.deviceId),connection:connections.get(b.deviceId)};};
const ensureClientContext=async({clientSessionId,agentId:aid,deviceId,workspace=null,gracePreset='60m'})=>{const b=clients.resolve(clientSessionId,{agentId:aid,deviceId:deviceId||null});const prior=b.workingContext||{},sessionId=prior.sessionId||`session-context-${b.deviceId}`,ctx=clients.setWorkingContext(clientSessionId,{agentId:aid,deviceId:b.deviceId,sessionId,workspace:workspace==null?(prior.workspace||''):workspace,gracePreset});const d=devices.get(b.deviceId);return {ok:true,context:{...ctx,nodeId:d.nodeId,displayName:d.displayName,deviceState:d.state,connectionState:connections.get(b.deviceId).state},session:{sessionId:ctx.sessionId,deviceId:b.deviceId,nodeId:d.nodeId,agentId:aid,workspace:ctx.workspace,state:'active',gracePreset:ctx.gracePreset}};};
const plus=createPlusAuth(wall,{pairAccess,pollAccess,assertGrant,attachClient,listClientDevices,resolveClientDevice,ensureClientContext});
...
r=response();await plus.connectPoll({body:{continuation:cont1}},r);assert.equal(r.statusCode,200);assert.equal(r.body.status,'ready');assert.equal(r.body.device,'VPS-AMD');assert.equal(r.body.context.deviceId,'dev-a');assert.equal(r.body.context.nodeId,'amd');assert.equal(r.body.context.gracePreset,'60m');const firstContextSession=r.body.context.sessionId;const client1=r.body.client;assert.ok(client1.startsWith('o1.client.'));assert.ok(JSON.stringify(r.body).length<1600);
r=response();await plus.connectPoll({body:{continuation:cont1}},r);assert.equal(r.statusCode,200);assert.equal(verify('client',r.body.client).clientSessionId,verify('client',client1).clientSessionId,'ready poll retry must be idempotent');assert.equal(r.body.context.sessionId,firstContextSession,'READY retry must reuse working session');
...
req=requestWithClient(client2,{workspace:'/tmp/project'});next=false;r=response();await plus.requireClient(req,r,()=>{next=true;});assert.ok(next);r=response();await plus.workingContext(req,r);assert.equal(r.statusCode,200);assert.equal(r.body.context.deviceId,'dev-b');assert.equal(r.body.context.sessionId,secondContextSession);assert.equal(r.body.context.workspace,'/tmp/project');
...
console.log('v10-pairing-ready-context=PASS');
