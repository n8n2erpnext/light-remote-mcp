...
Because RDC is approaching end-of-availability, beta promotion additionally requires Light Remote self-dogfood from the owner's actual ChatGPT Plus plan. Direct OAuth/MCP protocol proof has passed ARM/AMD/Windows and remains in the candidate for eligible Business/Enterprise/Edu testing and future Plugin/App publication, but it is not the Plus continuity lane. Plus must operate through `@Vercel` and a Vercel-authenticated protected Preview. The new `api/operator?via=plus` is GET-only at the ChatGPT-facing edge, rejects Production, then uses preview-scoped Vercel OIDC to reach dedicated ARM `/plus/*` routes; encrypted durable exec, signed device policy, capability inference and semantic idempotency remain authoritative. A fresh Plus + `@Vercel` end-to-end proof is required before the beta tag.
