...
assert.equal(reg.resolve(first.clientSessionId,{agentId:'agent-client-test-0001'}).deviceId,'dev-b','latest explicit pairing becomes exact default target');
const bound=reg.setWorkingContext(first.clientSessionId,{agentId:'agent-client-test-0001',deviceId:'dev-b',sessionId:'session-context-0001',workspace:'/tmp/work',gracePreset:'60m'});
assert.equal(bound.workspace,'/tmp/work');
assert.equal(reg.resolve(first.clientSessionId,{agentId:'agent-client-test-0001'}).workingContext.sessionId,'session-context-0001');
