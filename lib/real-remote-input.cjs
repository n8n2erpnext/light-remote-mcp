...
function screenIndex(value,name='screen'){
  if(value==null)return undefined;
  return integer(value,name,0,255);
}
function localizeCoordinates(coords,screen){
  if(screen==null)return coords;
  if(coords.x==null||coords.y==null)fail('desktop_input_coordinates_required');
  if(coords.x<0||coords.y<0)fail('desktop_input_invalid_screen_coordinates');
...
function normalizeEvent(value){
  ...
  if(type==='move'){
    ...
    return {type,...localizeCoordinates(coordinatePair(value),screenIndex(value.screen))};
  }
  if(type==='click'){
    const button=String(value.button||'left').trim().toLowerCase();
    if(!BUTTONS.has(button))fail('desktop_input_invalid_button');
    const coords=coordinatePair(value),screen=screenIndex(value.screen);
    return {type,button,count:integer(value.count??1,'count',1,3),...localizeCoordinates(coords,screen)};
  }
  if(type==='wheel'){
    const delta=integer(value.delta,'wheel',-1200,1200);
    if(delta===0)fail('desktop_input_invalid_wheel');
    const coords=coordinatePair(value),screen=screenIndex(value.screen);
    return {type,delta,...localizeCoordinates(coords,screen)};
  }
  if(type==='drag'){
    const start=coordinatePair(value);
    if(start.x==null||start.y==null)fail('desktop_input_coordinates_required');
    const button=String(value.button||'left').trim().toLowerCase();
    if(!BUTTONS.has(button))fail('desktop_input_invalid_button');
    const screen=screenIndex(value.screen);
    const toScreen=value.toScreen==null?screen:screenIndex(value.toScreen,'to_screen');
    const localizedStart=localizeCoordinates(start,screen);
    const toX=integer(value.toX,'to_x',-100000,100000),toY=integer(value.toY,'to_y',-100000,100000);
    if(toScreen!=null&&(toX<0||toY<0))fail('desktop_input_invalid_screen_coordinates');
    return {
      type,...localizedStart,toX,toY,
      ...(toScreen==null?{}:{toScreen}),
      button,
      steps:integer(value.steps??8,'steps',1,32),
      durationMs:integer(value.durationMs??120,'duration_ms',0,1000)
    };
  }
  if(type==='text'){
...
function normalizeDesktopInput(value){
  ...
  if(value.displayTopologyId!=null){
    const displayTopologyId=String(value.displayTopologyId||'').trim().toLowerCase();
    if(!/^[a-f0-9]{64}$/.test(displayTopologyId))fail('desktop_input_invalid_display_topology_id');
    result.displayTopologyId=displayTopologyId;
  }
