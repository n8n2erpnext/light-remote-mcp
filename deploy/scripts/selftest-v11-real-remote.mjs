...
const windowsCapabilityUpgradeAcceptance=read('deploy/scripts/selftest-v11-capability-upgrade-windows.mjs');
...
for(const token of ['function capabilityUpgradeView','discoveredCapabilities','missingCapabilities','capabilityUpgradeAvailable','capabilityUpgradePending'])assert.ok(agent.includes(token),'Capability upgrade status contract missing: '+token);
for(const token of ['New capabilities available.','Re-authorize device','missingCapabilities','/api/enrollment/begin','/api/enrollment/poll','pollCapabilityUpgrade'])assert.ok(wall.includes(token),'Capability upgrade Permissions UI missing: '+token);
for(const token of ['desktop','desktop-input','capabilityUpgradeAvailable','capability_upgrade_must_fail_closed_before_reauth','windows-capability-upgrade-discovery=PASS'])assert.ok(windowsCapabilityUpgradeAcceptance.includes(token),'Windows capability upgrade acceptance missing: '+token);
assert.ok(windowsWorkflow.includes('- name: Capability upgrade discovery acceptance')&&windowsWorkflow.includes('selftest-v11-capability-upgrade-windows.mjs $exe')&&!windowsWorkflow.slice(windowsWorkflow.indexOf('- name: Capability upgrade discovery acceptance'),windowsWorkflow.indexOf('- name: Local Wall desktop viewer native acceptance')).includes('continue-on-error'),'Windows capability upgrade acceptance must be a hard gate');
...
console.log('v11-real-remote-capability-upgrade=PASS');
