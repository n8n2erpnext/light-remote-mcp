...
4. Send `agentId + sessionId` on all operator exec/job/output/resume calls and on tracked read calls.
5. On transient network loss, resume the same session with the same agentId; if it has expired, open a new lane.
6. Prefer grouped `exec_batch` operations when commands logically belong together.
...
## Security / execution
v0.6 removes the static shared Bearer boundary from the development Vercel transport. Vercel OIDC continues to authenticate Vercel -> ARM, while privileged bodies remain protected with X25519 + HKDF-SHA256 + AES-256-GCM, replay rejection and semantic `operationId` idempotency. The eventual public App/Plugin authorization layer is account/device/permission based, not a long-lived shared Bearer secret.
...
Use the Vercel bridge directly for normal development calls; v0.6 does not require a static shared caller Bearer. Preview Deployment Protection may still gate preview URLs independently at the Vercel platform layer. Self-disruptive gateway work uses RDC only for the disruptive step.
