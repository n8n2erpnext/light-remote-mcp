...
let pairingCalls=0,accountAuthCalls=0;
...
try{
  ...
  r=await req('GET','/login',{headers:{host:'100.94.184.141:5491'}});assert.equal(r.status,200);assert.match(r.text,/Device Wall login/);let csrf=loginCsrf(r.text);
  r=await req('POST','/auth/login',{body:new URLSearchParams({username:'operator',password,next:'/'}).toString(),headers:{host:'100.94.184.141:5491'}});assert.equal(r.status,403);assert.ok(!r.headers['set-cookie']);
  r=await req('POST','/auth/login',{body:new URLSearchParams({username:'operator',password:'wrong',next:'/',csrf}).toString(),headers:{host:'100.94.184.141:5491'}});assert.equal(r.status,401);assert.ok(!r.headers['set-cookie']);
  r=await req('POST','/auth/login',{body:new URLSearchParams({username:'operator',password,next:'/',csrf}).toString(),headers:{host:'100.94.184.141:5491'}});assert.equal(r.status,303);
  ...
  r=await req('GET','/',{headers:{cookie,host:'100.94.184.141:5491'}});assert.equal(r.status,200);assert.match(r.text,/LIVE OPERATOR STREAM/);const mutation=mutationCsrf(r.text);
  r=await req('POST','/api/pairing-code',{headers:{cookie,host:'100.94.184.141:5491','x-light-remote-csrf':mutation,'content-type':'application/json'},body:'{}'});assert.equal(r.status,200);assert.equal(pairingCalls,1);assert.match(r.text,/ABCD-EFGH/);
  ...
  r=await req('POST','/auth/logout',{headers:{cookie,host:'100.94.184.141:5491'},body:new URLSearchParams({csrf:mutation}).toString()});assert.equal(r.status,303);assert.match(String(r.headers['set-cookie']),/Max-Age=0/);
