...
for(const token of ['DesktopSessions','DesktopSessionLimit = 8','desktopSessionId','FrameSeq','desktop_session_stale_topology','NextDesktopFrameSeq'])assert.ok(desktopSessionHelper.includes(token),`Windows visual desktop session contract missing: ${token}`);
