...
- Full static regression after this work is 39/39 PASS; root and gateway audits remain zero known vulnerabilities. Native OAuth dogfood now covers ARM, AMD and Windows plus typed file/list/search/process convenience tools, including a harmless spawn/terminate proof. Reference ARM production now serves `0.9.0-beta.1` OAuth MCP at `https://mcp.dashboard.thaiduy.store/mcp`; the full public proof passed ARM/AMD/Windows including typed convenience file/process operations, and post-proof fleet active sessions returned to zero. Fresh-ChatGPT custom MCP App acceptance is the next gate before beta promotion.
