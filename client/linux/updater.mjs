...
function parseSystemctlState(r){
  const values=Object.fromEntries(String(r.stdout||'').split('\n').filter(Boolean).map(line=>{const i=line.indexOf('=');return i>0?[line.slice(0,i),line.slice(i+1)]:[line,''];}));
  const pid=Number(values.MainPID)||0;
  return {healthy:r.status===0&&values.ActiveState==='active'&&values.SubState==='running'&&pid>0,pid,activeState:values.ActiveState||'',subState:values.SubState||''};
}
function userContexts(){
  if(!USER_UNITS)return[];let dirs=[];try{dirs=fs.readdirSync('/run/user',{withFileTypes:true});}catch{return[];}
  const out=[];for(const d of dirs){if(!d.isDirectory()||!/^\d+$/.test(d.name))continue;const uid=Number(d.name),runtime=`/run/user/${uid}`,bus=`${runtime}/bus`;if(!fs.existsSync(bus))continue;const pw=run('getent',['passwd',String(uid)],{allowFailure:true});if(pw.status!==0)continue;const user=String(pw.stdout||'').split(':')[0];if(user)out.push({uid,user,runtime,bus});}return out;
}
...
function serviceStates(){
  if(!USER_UNITS)return [parseSystemctlState(run('systemctl',['show',SERVICE,'--property=ActiveState','--property=SubState','--property=MainPID','--no-pager'],{allowFailure:true}))];
  return userContexts().map(ctx=>({...parseSystemctlState(userSystemctl(ctx,['show',SERVICE,'--property=ActiveState','--property=SubState','--property=MainPID','--no-pager'],{allowFailure:true})),uid:ctx.uid,user:ctx.user}));
}
function restartServices(){
  if(!USER_UNITS){run('systemctl',['daemon-reload'],{allowFailure:true});return run('systemctl',['restart',SERVICE],{allowFailure:true}).status===0;}
  const contexts=userContexts();let ok=true;for(const ctx of contexts){userSystemctl(ctx,['daemon-reload'],{allowFailure:true});if(userSystemctl(ctx,['restart',SERVICE],{allowFailure:true}).status!==0)ok=false;}return ok;
}
async function stableServiceHealthy(timeoutMs=12000,stableMs=3000){
  const deadline=Date.now()+timeoutMs;let healthySince=0;
  while(Date.now()<deadline){const states=serviceStates(),healthy=states.length===0||states.every(x=>x.healthy);if(healthy){if(!healthySince)healthySince=Date.now();if(Date.now()-healthySince>=stableMs)return true;}else healthySince=0;await new Promise(r=>setTimeout(r,250));}
  return false;
}
...
async function main(){
  ...
  try{
    ...
    const healthy=restarted&&await stableServiceHealthy();
    if(!healthy){
      let rollbackHealthy=false;
      if(previous&&fs.existsSync(previous)){atomicCurrent(previous);const rollbackRestart=restartServices();rollbackHealthy=rollbackRestart&&await stableServiceHealthy(15000,3000);}
      if(!rollbackHealthy)fail(`update_rollback_failed:${manifest.version}`);
