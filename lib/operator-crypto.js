...
const bundledPublicKeys = require('../operator-public-keys.json');
function loadPublicKeys() {
  let raw = '';
  try { raw = globalThis.Netlify?.env?.get?.('OPERATOR_PUBLIC_KEYS_JSON') || ''; } catch {}
  raw = raw || process.env.OPERATOR_PUBLIC_KEYS_JSON;
  if (!raw) return bundledPublicKeys;
  let parsed; try { parsed = JSON.parse(raw); } catch { throw new Error('operator_public_keys_json_invalid'); }
