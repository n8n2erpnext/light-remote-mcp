...
const FINISHED_TTL_MS=6*60*60*1000,DEFAULT_IDLE_TTL_MS=30*60*1000,DEFAULT_HARD_TTL_MS=4*60*60*1000,DEFAULT_STOP_GRACE_MS=750;
...
export class NativeTerminalRegistry {
  constructor({maxTerminals=8,bufferBytes=DEFAULT_BUFFER_BYTES,now=()=>Date.now(),emit=()=>{},platform=process.platform,ptyProvider=null,idleTtlMs=DEFAULT_IDLE_TTL_MS,hardTtlMs=DEFAULT_HARD_TTL_MS,stopGraceMs=DEFAULT_STOP_GRACE_MS}={}){
    this.maxTerminals=Math.max(1,Math.min(Number(maxTerminals)||8,32));this.bufferBytes=bufferBytes;this.now=now;this.emit=emit;this.platform=platform;this.ptyProvider=ptyProvider;this.idleTtlMs=Math.max(60000,Number(idleTtlMs)||DEFAULT_IDLE_TTL_MS);this.hardTtlMs=Math.max(this.idleTtlMs,Number(hardTtlMs)||DEFAULT_HARD_TTL_MS);this.stopGraceMs=Math.max(50,Math.min(Number(stopGraceMs)||DEFAULT_STOP_GRACE_MS,5000));this.rows=new Map();
  ...
  _finalize(row,{exitCode=null,signal=null,state=null}={}){if(row.finishedAt)return;row.finishedAt=this.now();row.lastActivityAt=row.finishedAt;row.exitCode=Number.isInteger(exitCode)?exitCode:null;row.signal=signal==null?null:String(signal);if(row.stopTimer){clearTimeout(row.stopTimer);row.stopTimer=null;}row.state=state||(row.state==='expired'?'expired':row.state==='stopping'?'finished':row.exitCode===0?'finished':'error');this.emit({type:'terminal_finished',accountId:row.accountId,deviceId:row.deviceId,sessionId:row.sessionId,agentId:row.agentId,terminalId:row.terminalId,state:row.state,exitCode:row.exitCode,signal:row.signal,durationMs:row.finishedAt-row.startedAt});}
  _syncExit(row){if(row.finishedAt||!Number.isInteger(row.pid)||row.pid<=0)return;try{process.kill(row.pid,0);}catch(error){if(error?.code==='ESRCH')this._finalize(row,{signal:row.stopSignal||null,state:row.state==='expired'?'expired':'finished'});}}
  _kill(row,signal){try{row.pty.kill(this.platform==='win32'?undefined:signal);}catch(error){throw new NativeTerminalError(`terminal_signal_failed:${error.message}`,409);}}
  prune(){const now=this.now();for(const [id,row] of this.rows){this._syncExit(row);if(row.finishedAt&&now-row.finishedAt>FINISHED_TTL_MS){this.rows.delete(id);continue;}if(!row.finishedAt&&(now-row.startedAt>this.hardTtlMs||now-row.lastActivityAt>this.idleTtlMs)){row.state='expired';row.stopSignal='SIGKILL';try{this._kill(row,'SIGKILL');}catch{}setTimeout(()=>this._syncExit(row),50).unref?.();}}}
  start({accountId,deviceId,sessionId,agentId,shellSpec,cwd,env={},cols=120,rows=32,term='xterm-256color'}){
    ...
    pty.onExit(event=>this._finalize(row,{exitCode:event?.exitCode,signal:event?.signal}));
  ...
  stop(terminalId,owner,{force=false}={}){const row=this._row(terminalId,owner);this._syncExit(row);if(row.finishedAt)return this.view(terminalId,owner);row.state='stopping';row.stopSignal=force?'SIGKILL':'SIGTERM';this._kill(row,row.stopSignal);row.lastActivityAt=this.now();this.emit({type:'terminal_signal',...owner,terminalId,signal:force?'kill':'terminate'});if(force){setTimeout(()=>this._syncExit(row),50).unref?.();}else{row.stopTimer=setTimeout(()=>{if(row.finishedAt)return;row.stopSignal='SIGKILL';try{this._kill(row,'SIGKILL');}catch{}setTimeout(()=>this._syncExit(row),50).unref?.();},this.stopGraceMs);row.stopTimer.unref?.();}return this.view(terminalId,owner);}
  view(terminalId,owner=null){const row=this._row(terminalId,owner);this._syncExit(row);return {terminalId:row.terminalId,accountId:row.accountId,deviceId:row.deviceId,sessionId:row.sessionId,agentId:row.agentId,pid:row.pid,state:row.state,shell:row.shell,cwd:row.cwd,cols:row.cols,rows:row.rows,startedAt:row.startedAt,lastActivityAt:row.lastActivityAt,firstOutputAt:row.firstOutputAt,finishedAt:row.finishedAt,exitCode:row.exitCode,signal:row.signal,outputBytes:row.output.totalBytes};}
  ...
  close(){if(this.timer)clearInterval(this.timer);for(const row of this.rows.values())if(!row.finishedAt)try{row.stopSignal='SIGKILL';this._kill(row,'SIGKILL');}catch{}}
}
