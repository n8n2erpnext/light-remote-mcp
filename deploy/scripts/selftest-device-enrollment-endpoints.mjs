...
const approved=await request('POST','/v1/enrollments/approve',{code:e.deviceCode,accountId:'self-hosted-local',approvedCapabilities:['git','docker'],displayName:'Approved Endpoint Device',policyProfile:'test-policy'});
...
let devices=await request('GET','/v1/devices');
let enrolled=devices.json.devices?.find(d=>d.deviceId===deviceId);
if(!enrolled||enrolled.state!=='online'||enrolled.nodeId!==deviceId||enrolled.publicIdentityKey!==publicIdentityKey||enrolled.policy?.policyRevision!==1||!enrolled.policy?.grantableCapabilities?.includes('docker'))throw new Error('enrolled_device_view_failed');
const policy=await request('POST',`/v1/devices/${deviceId}/policy`,{deviceId,accountId:'self-hosted-local',approvedCapabilities:['git'],policyProfile:'restricted'});
...
const refreshTimestamp=Date.now(),refreshNonce=crypto.randomBytes(18).toString('base64url'),supportedCapabilities=['desktop','docker','git'],refreshCapabilities=['git'];
const refreshSignature=crypto.sign(null,Buffer.from(deviceHeartbeatMessage({deviceId,timestamp:refreshTimestamp,nonce:refreshNonce,capabilities:refreshCapabilities,supportedCapabilities})),privateKey).toString('base64url');
const refreshed=await request('POST',`/v1/devices/${deviceId}/heartbeat`,{deviceId,timestamp:refreshTimestamp,nonce:refreshNonce,capabilities:refreshCapabilities,supportedCapabilities,signature:refreshSignature,policyRevision:2});
if(refreshed.status!==200||refreshed.json.device?.deviceId!==deviceId||refreshed.json.device?.capabilities?.join(',')!=='git'||refreshed.json.policy?.policy?.policyRevision!==3)throw new Error('capability_refresh_heartbeat_failed:'+refreshed.status+':'+String(refreshed.json.error||''));
if(!refreshed.json.policy.policy.grantableCapabilities.includes('desktop')||!refreshed.json.policy.policy.approvedCapabilities.includes('desktop')||refreshed.json.policy.policy.approvedCapabilities.includes('docker'))throw new Error('capability_refresh_policy_sync_failed');
devices=await request('GET','/v1/devices');enrolled=devices.json.devices?.find(d=>d.deviceId===deviceId);
if(enrolled?.deviceId!==deviceId||enrolled?.policy?.policyRevision!==3||!enrolled?.policy?.grantableCapabilities?.includes('desktop')||!enrolled?.policy?.approvedCapabilities?.includes('desktop'))throw new Error('capability_refresh_same_identity_view_failed');
const enabledTimestamp=Date.now(),enabledNonce=crypto.randomBytes(18).toString('base64url'),enabledCapabilities=['desktop','git'];
const enabledSignature=crypto.sign(null,Buffer.from(deviceHeartbeatMessage({deviceId,timestamp:enabledTimestamp,nonce:enabledNonce,capabilities:enabledCapabilities,supportedCapabilities})),privateKey).toString('base64url');
const enabledHeart=await request('POST',`/v1/devices/${deviceId}/heartbeat`,{deviceId,timestamp:enabledTimestamp,nonce:enabledNonce,capabilities:enabledCapabilities,supportedCapabilities,signature:enabledSignature,policyRevision:3});
if(enabledHeart.status!==200||enabledHeart.json.device?.deviceId!==deviceId||enabledHeart.json.device?.capabilities?.join(',')!=='desktop,git'||enabledHeart.json.policy!=null)throw new Error('capability_refresh_local_enable_failed');
const pendingAfterRefresh=await request('GET','/v1/enrollments');
if(pendingAfterRefresh.status!==200||(pendingAfterRefresh.json.pending||[]).length!==0)throw new Error('capability_refresh_must_not_create_enrollment');
console.log('device-capability-refresh-same-identity=PASS');
console.log('device-capability-refresh-no-reauthorize=PASS');
...
const firstSeenAt=enrolled.firstSeenAt;
const rebegun=await request('POST','/v1/enrollments/begin',{publicIdentityKey,displayName:'Endpoint Device',platform:'linux',architecture:'x64',agentVersion:'0.7-test',fingerprintSummary:'endpoint/linux/x64',capabilities:['git','docker'],policyProfile:'test-policy'});
if(rebegun.status!==200||!rebegun.json.enrollment?.deviceCode)throw new Error('reenroll_begin_failed');
const reapproved=await request('POST','/v1/enrollments/approve',{code:rebegun.json.enrollment.deviceCode,accountId:'self-hosted-local',approvedCapabilities:['git','docker'],displayName:'Approved Endpoint Device',policyProfile:'test-policy'});
