...
assert.ok(windowsWorkflow.includes('- name: Capability refresh without reauthorization acceptance')&&windowsWorkflow.includes('selftest-v11-capability-refresh-windows.mjs $exe')&&!windowsWorkflow.slice(windowsWorkflow.indexOf('- name: Capability refresh without reauthorization acceptance'),windowsWorkflow.indexOf('- name: Local Wall desktop viewer native acceptance')).includes('continue-on-error'),'Windows capability refresh acceptance must be a hard gate');
