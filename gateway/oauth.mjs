...
:root{color-scheme:dark;font-family:ui-sans-serif,system-ui;background:#090b0e;color:#e5e7eb}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center}.card{width:min(460px,calc(100vw - 32px));border:1px solid #2b3139;border-radius:16px;background:#10141a;padding:24px}.brand-title{display:flex;align-items:center;gap:12px;margin-bottom:10px}.brand-title>span{display:flex;align-items:baseline;gap:7px}.brand-title strong{font-size:20px;color:#f3f4f6}.brand-title small{font-size:10px;letter-spacing:.16em;color:#98a2b3}.brand-mark{flex:0 0 auto}.muted{color:#98a2b3}.err{color:#ffaaaa;margin:12px 0}label{display:block;margin:14px 0}input,button{width:100%;padding:11px;border-radius:8px;border:1px solid #303844;background:#0b0f14;color:#e5e7eb;font:inherit}button{cursor:pointer;background:#1d4ed8;border-color:#1d4ed8;margin-top:8px}</style></head><body><main class="card">${brandTitleSvg(48)}<h1>Authorize Light Remote MCP</h1><p class="muted">ChatGPT is requesting access to your policy-bounded remote devices. Device policy and local capability checks remain authoritative.</p>${error}<form method="post" action="/oauth/authorize">${hidden}<label>Username<input name="username" autocomplete="username" required></label><label>Password<input type="password" name="password" autocomplete="current-password" required></label><button type="submit">Authorize</button></form></main></body></html>`;
}
