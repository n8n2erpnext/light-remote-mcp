...
function serviceState(){
  const r=run('systemctl',['show',SERVICE,'--property=ActiveState','--property=SubState','--property=MainPID','--no-pager'],{allowFailure:true});
...
async function stableServiceHealthy(timeoutMs=12000,stableMs=3000){
  ...
  while(Date.now()<deadline){const state=serviceState();if(state.healthy){if(!healthySince)healthySince=Date.now();if(Date.now()-healthySince>=stableMs)return true;}else healthySince=0;await new Promise(r=>setTimeout(r,250));}
  return false;
}
...
async function main(){
  ...
  try{
    ...
    const healthy=restarted.status===0&&await stableServiceHealthy();
    if(!healthy){
      let rollbackHealthy=false;
      if(previous&&fs.existsSync(previous)){atomicCurrent(previous);run('systemctl',['daemon-reload'],{allowFailure:true});const rollbackRestart=run('systemctl',['restart',SERVICE],{allowFailure:true});rollbackHealthy=rollbackRestart.status===0&&await stableServiceHealthy(15000,3000);}
