...
function execPayload(value){
  ...
  return { action:'exec_batch', operationId, script:d.script, cwd:d.cwd||'/home/ubuntu',
    ...
    waitMs:Math.max(0,Math.min(Number.isFinite(Number(d.waitMs))?Number(d.waitMs):7000,7000)), sessionId:sid(d.sessionId), agentId:aid(d.agentId), note:String(d.note||'') };
}
module.exports=async function handler(req,res){
  ...
  try {
    ...
    else if(action==='job') upstream=await callOperator(`/operator/jobs/${encodeURIComponent(id(req.query.id))}?agentId=${encodeURIComponent(aid(req.query.aid))}`);
    else if(action==='output') {
      const q=new URLSearchParams({agentId:aid(req.query.aid),stream:req.query.stream==='stderr'?'stderr':'stdout',full:req.query.full==='1'?'1':'0',
    ...
    console.log(JSON.stringify({event:'operator_bridge',action,sessionId:req.query.sid||upstream?.session?.sessionId||upstream?.job?.sessionId||null,agentId:req.query.aid||upstream?.session?.agentId||upstream?.job?.agentId||null,nodeId:upstream?.session?.nodeId||upstream?.job?.nodeId||'arm',status:200,durationMs:Date.now()-started}));
    return res.status(200).json({ok:true,bridge:'vercel',action,upstream});
  } catch(e){ const status=e.status||400; console.warn(JSON.stringify({event:'operator_bridge',action,sessionId:req.query.sid||null,agentId:req.query.aid||null,status,error:e.message,durationMs:Date.now()-started})); return res.status(status).json({ok:false,error:e.message,upstream:e.payload||null}); }
};
