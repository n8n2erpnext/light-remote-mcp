...
async function accountAction(req,res,action){
  ...
  if(action==='device-update'){
    ...
    const upstream=await callOperator(`/account/devices/${encodeURIComponent(id)}/update`,{method:'POST',body:{force:true},accountSession:token,timeoutMs:9000});
    return res.status(200).json(upstream);
  }
  ...
  if(action==='main-device'){
    ...
    const upstream=await callOperator('/account/main-device',{method:'POST',body:{deviceId},accountSession:token,timeoutMs:9000});
    return res.status(200).json(upstream);
  }
  if(action==='main-device-clear'){
    if(!method(req,res,'POST'))return; const token=accountRequired(req,res); if(!token)return;
    const upstream=await callOperator('/account/main-device/clear',{method:'POST',body:{},accountSession:token,timeoutMs:9000});
    return res.status(200).json(upstream);
  }
  if(action==='redeem-license'){
    ...
    const upstream=await callOperator('/account/redeem-license',{method:'POST',body:{key},accountSession:token,timeoutMs:9000});
    return res.status(200).json(upstream);
  }
