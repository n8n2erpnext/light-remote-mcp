...
const packageDir=path.join(tmp,'src','fleet-wall');fs.mkdirSync(path.join(packageDir,'device-agent'),{recursive:true});fs.mkdirSync(path.join(packageDir,'lib'),{recursive:true});fs.mkdirSync(path.join(packageDir,'gateway'),{recursive:true});fs.mkdirSync(path.join(packageDir,'assets','branding'),{recursive:true});fs.mkdirSync(path.join(packageDir,'assets','fonts'),{recursive:true});
for(const [src,dst] of [['device-agent/fleet-wall-runtime.mjs','device-agent/fleet-wall-runtime.mjs'],['device-agent/local-wall-auth.mjs','device-agent/local-wall-auth.mjs'],['lib/device-proof.mjs','lib/device-proof.mjs'],['lib/runtime-version.mjs','lib/runtime-version.mjs'],['lib/brand.mjs','lib/brand.mjs'],['gateway/dashboard.mjs','gateway/dashboard.mjs'],['gateway/device-policy-page.mjs','gateway/device-policy-page.mjs'],['gateway/brand.mjs','gateway/brand.mjs'],['assets/branding/light-remote-mark.svg','assets/branding/light-remote-mark.svg'],['assets/fonts/CascadiaMono.ttf','assets/fonts/CascadiaMono.ttf'],['assets/fonts/CascadiaMono-OFL.txt','assets/fonts/CascadiaMono-OFL.txt']])fs.copyFileSync(path.join(root,src),path.join(packageDir,dst));
