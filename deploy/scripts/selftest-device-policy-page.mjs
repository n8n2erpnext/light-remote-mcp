...
if(!html.includes('Device permissions')||!html.includes('Save permissions')||!html.includes('Files & folders')||!html.includes('Administrator (sudo)')||!html.includes('account-approved ceiling')||!html.includes('local policy')||!html.includes('Run signed update now')||!html.includes('/api/devices/'))throw new Error('device_permissions_page_contract_missing');
if(html.includes('Policy profile<input type="text"')||!html.includes('type="hidden" id="profile"')||!html.includes('p.grantableCapabilities'))throw new Error('device_permissions_must_hide_profile_and_bound_to_grantable');
...
if(!sync.includes('context: "${ROOT_DIR}"')||!sync.includes('dockerfile: gateway/Dockerfile')||!sync.includes('docker compose build "$SERVICE"'))throw new Error('gateway_sync_device_policy_page_missing');
const dockerfile=fs.readFileSync(new URL('../../gateway/Dockerfile',import.meta.url),'utf8');
if(!dockerfile.includes('COPY gateway/*.mjs ./'))throw new Error('gateway_image_device_policy_page_missing');
