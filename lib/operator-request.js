...
function normalizeSessionOpenPayload(d) {
  ...
  return { openId, agentId:aid(d.agentId), label:String(d.label||''), workspace:String(d.workspace||''), leaseMs, leasePreset };
}
...
function normalizeDeviceHeartbeat(d) {
  ...
  return { deviceId:validId(d.deviceId,/^dev_[a-f0-9]{24}$/,'invalid_device_id'), timestamp, nonce, signature, capabilities:cleanCapabilityList(d.capabilities) };
}
