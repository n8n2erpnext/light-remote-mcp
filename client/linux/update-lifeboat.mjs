...
function helperReleaseFiles(sourceRoot,target){copyFile(path.join(sourceRoot,'runtime','node'),path.join(target,'runtime','node'),0o755);copyFile(path.join(sourceRoot,'client','linux','updater.mjs'),path.join(target,'client','linux','updater.mjs'),0o644);copyFile(path.join(sourceRoot,'client','linux','update-lifeboat.mjs'),path.join(target,'client','linux','update-lifeboat.mjs'),0o644);copyFile(path.join(sourceRoot,'lib','update-contract.mjs'),path.join(target,'lib','update-contract.mjs'),0o644);copyFile(path.join(sourceRoot,'manifest.json'),path.join(target,'manifest.json'),0o644);}
...
export function finalizeHelperFromCore(paths,sourceRoot,{env=process.env}={}){
  const manifest=helperManifest(sourceRoot),version=String(manifest.version),target=path.join(paths.helperReleases,version),tmp=path.join(paths.helperReleases,`.incoming-${version}-${process.pid}`);
  ...
  const smoke=spawnSync(path.join(tmp,'runtime','node'),[path.join(tmp,'client','linux','updater.mjs'),'--self-test'],{encoding:'utf8',env:{...env,GPT_OPERATOR_INSTALL_ROOT:paths.root,LIGHT_REMOTE_UPDATE_STATE_DIR:paths.stateDir,LIGHT_REMOTE_UPDATER_ROOT:paths.updaterRoot}});
