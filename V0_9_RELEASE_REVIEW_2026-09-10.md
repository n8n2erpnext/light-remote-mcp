...
Because RDC is approaching end-of-availability, beta promotion additionally requires Light Remote self-dogfood from the owner's actual ChatGPT Plus plan. Direct OAuth/MCP protocol proof has passed ARM/AMD/Windows and remains in the candidate for eligible Business/Enterprise/Edu testing and future Plugin/App publication, but it is not the Plus continuity lane. Plus must operate through `@Vercel`. The `api/operator?via=plus` compatibility lane is GET-only at the ChatGPT-facing edge. It bootstraps devices before authorization; the selected device either reuses an active Device Access Grant or receives a ten-minute request on its Local Wall. The enrolled device service signs Approve/Deny, and `authorize-poll` returns a capability scoped to that grant/device/connection and bounded by the Device Connection Lease. Production or Preview Vercel functions still present environment-scoped OIDC to dedicated ARM `/plus/*` routes; ARM additionally verifies the Device Access Grant before device/session/exec actions. Encrypted durable exec, signed device policy, capability inference and semantic idempotency remain authoritative. A fresh Plus + `@Vercel` pairing/execution proof is required before the beta tag.
