...
const hub=http.createServer(async(req,res)=>{let text='';for await(const chunk of req)text+=chunk;const body=JSON.parse(text||'{}'),action=String(req.url||'').split('/').pop();
  ...
  if(action==='fleet-activity')return reply(res,200,{ok:true,mainDeviceId:deviceId,events:[{id:1,at:new Date().toISOString(),type:'device_online',deviceId,accountId:'self-hosted-local'}]});
...
let r=await get(fleetPort,'/');if(r.status!==200||!r.text.includes('Fleet Wall · Main device')||!r.text.includes('const surfaceFleet=true'))throw new Error('fleet_wall_dashboard_contract_failed');
