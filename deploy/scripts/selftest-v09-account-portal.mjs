...
need(auth.includes("action==='register'")&&auth.includes('ownerCode')&&auth.includes('ownerPassword')&&auth.includes("action==='device-revoke'")&&auth.includes("action==='device-remove'")&&auth.includes("action==='devices-revoke-all'")&&auth.includes("action==='redeem-license'"),'account-auth-router');
const gateway=read('gateway/server.mjs'),operator=read('operator-host/executor.mjs');need(gateway.includes('/account/devices/:id/remove'),'account-device-remove-route');need(gateway.includes('deviceChannelEdgeRateLimit')&&!gateway.includes('deviceRateIdentity')&&gateway.includes('mainDeviceRateLimit')&&operator.includes('trustedChannelLane')&&operator.includes('DeviceChannelRateLimitError'),'gateway-scoped-rate-limits');need(gateway.includes('ownerCode')&&gateway.includes('owner_migration_proof_required')&&gateway.includes('wallAuth.verifyCredentials'),'account-owner-migration-proof');
