...
function auth(){const s=env('NETLIFY_BRIDGE_SECRET')||env('LIGHT_REMOTE_N_BRIDGE_SECRET')||String(runtimeConfig.bridgeSecret||'');if(!s)throw new Error('netlify_bridge_secret_unavailable');return {'x-light-netlify-bridge':s}}
async function callOperator(path,{method='GET',body=null,timeoutMs=9000,bridgeSession='',plusSession='',plusClient='',accountSession=''}={}){
 const headers={accept:'application/json',...auth()};if(bridgeSession)headers['x-bridge-session']=bridgeSession;if(plusSession)headers['x-plus-session']=plusSession;if(plusClient){headers['x-light-client']=plusClient;headers.cookie='light_remote_client='+encodeURIComponent(plusClient);}if(accountSession)headers['x-light-account-session']=accountSession;if(body!=null)headers['content-type']='application/json';
