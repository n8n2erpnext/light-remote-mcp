...
function toolHelperView(contextEnvelope={}){
  ...
  return {
    ...
    tools:{
      ...
      desktop:{actions:['desktop-status','desktop-windows','desktop-frame','desktop-semantic-attach','desktop-semantic-snapshot','desktop-semantic-detach'],when:'Read the actual interactive desktop state on a target that advertises desktop capability.',capability:'desktop',payload:`${common.replace('...','limit?,screen?,maxWidth?,maxHeight?,quality?,scope?,maxDepth?,maxNodes?,semanticSessionId?')}`,ops:{status:'Read interactive session, screens, cursor and foreground window.',windows:'List visible top-level windows; optional limit 1..200.',frame:'Capture one bounded JPEG frame from the real Windows desktop. Defaults max 960x540 quality 50; hard limits 1280x720 and quality 25..70.',semanticAttach:'Open a bounded Windows UI Automation semantic session; scope foreground|desktop, defaults foreground.',semanticSnapshot:'Refresh the bounded UIA tree for semanticSessionId and advance stateSeq.',semanticDetach:'Close semanticSessionId.'},rule:'The desktop view path is read-only. Semantic sessions use epoch/stateSeq, expose control metadata/bounds/patterns, and do not read textbox or password contents. desktop-frame remains a visual checkpoint, not the primary reasoning channel.'},
