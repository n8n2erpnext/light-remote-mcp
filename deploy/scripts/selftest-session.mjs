...
const child=spawn(process.execPath,[`${root}/operator-host/executor.mjs`],{cwd:root,env:{...process.env,OPERATOR_SOCKET:socketPath,OPERATOR_LOG_DIR:logDir,OPERATOR_KEY_FILE:'/home/ubuntu/.config/gpt-vps-operator/operator.private.json',OPERATOR_SESSION_IDLE_MS:'800',OPERATOR_MAX_ACTIVE_SESSIONS:'3',OPERATOR_SESSION_HISTORY_MS:'10000',OPERATOR_NODE_ID:'arm'},stdio:['ignore','pipe','pipe']});
...
const opened=await request('POST','/v1/sessions/open',{openId,agentId:aid,label:'long-build',workspace:'/home/ubuntu'}); if(opened.status!==200) throw new Error('open_failed'); const sid=opened.json.session.sessionId;
...
await sleep(900); const expired=await request('GET',`/v1/sessions/${sid}?agentId=${aid}`); if(expired.json.session.state!=='expired') throw new Error('expiry_failed');
const a=await request('POST','/v1/sessions/open',{agentId:'agent-cap-a-v05-aaaaaaaa'}), b=await request('POST','/v1/sessions/open',{agentId:'agent-cap-b-v05-bbbbbbbb'}), c=await request('POST','/v1/sessions/open',{agentId:'agent-cap-c-v05-cccccccc'}), d=await request('POST','/v1/sessions/open',{agentId:'agent-cap-d-v05-dddddddd'}); if([a,b,c].some(x=>x.status!==200)||d.status!==429) throw new Error('capacity_failed');
const stats=await request('GET','/v1/session-stats?hours=1'); const row=stats.json.sessions?.find(x=>x.sessionId===sid); if(!row||row.execCalls<1||row.holdStarts!==1||row.holdReleases!==1) throw new Error('session_stats_failed');
console.log(JSON.stringify({ok:true,nodeId:opened.json.session.nodeId,sessionId:sid,agentId:aid,wrongAgentStatus:wrongResume.status,oneAgentOneSession:sameAgent.json.session.sessionId===sid,held:held.json.session.state,released:released.json.session.state,expired:expired.json.session.state,capacityStatus:d.status},null,2)); child.kill('SIGTERM'); await sleep(100);
