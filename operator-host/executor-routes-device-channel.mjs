export async function handleDeviceChannelRoutes(req,res,url,deps){
  const {ACCOUNT_ID,AccountError,AgentClientRegistryError,CLIENT_BACKWARD_RELEASES,CONNECTION_LEASE_ENFORCE,DeviceAccessGrantError,DevicePairingRegistryError,EnrollmentError,FleetAuthorityError,FleetError,FLEET_WALL_PORT=5492,MAX_MEMORY_OUTPUT,MAX_RING_EVENTS,MIN_SUPPORTED_CLIENT_VERSION,NODE_ID,VERSION,accessGrants,accounts,agentClients,allDeviceViews,applyDeviceTelemetry,capabilities,clientCompatibility,connectionSpec,connections,deviceView,devices,emitStream,enrollments,finishJob,fleet,fleetAuthority,fleetEligibility,fleetTarget,jobView,jobs,normalizeUpdateReport,pairingCodes,planEntitlements,pushEvent,queueHelperUpdate,readJson,reapAccessGrants,recentEvents,requireDeviceConnection,sendJson,sessions,targetRoute,terminalResultSummary,verifiedChannelContext,verifiedFleetContext,verifiedLeafCapabilities}=deps;
    ...
    if (req.method === 'POST' && url.pathname === '/v1/device-channel/fleet-intent') {
      ...
      const observedFleetHealthy=ctx.payload.fleetHealthy===true&&Number(ctx.payload.fleetPort)===FLEET_WALL_PORT;
      if(desired&&observedFleetHealthy)account=accounts.setFleetProvisioning(account.accountId,{deviceId:ctx.device.deviceId,state:'online',reason:'fleet_runtime_observed',moduleVersion:ctx.payload.moduleVersion||null,...versionState,port:FLEET_WALL_PORT});
      else if(desired&&account.fleetProvisioning?.deviceId===ctx.device.deviceId&&account.fleetProvisioning?.state!=='online')account=accounts.setFleetProvisioning(account.accountId,{deviceId:ctx.device.deviceId,state:'configuring',reason,moduleVersion:ctx.payload.moduleVersion||null,...versionState,port:FLEET_WALL_PORT});
      else if(!desired&&account.mainDeviceId===ctx.device.deviceId)account=accounts.setFleetProvisioning(account.accountId,{deviceId:ctx.device.deviceId,state:compatibility.updateRequired?'update_required':'failed',reason,moduleVersion:ctx.payload.moduleVersion||null,...versionState,port:FLEET_WALL_PORT});
      return sendJson(res,200,{ok:true,fleet:{desired,port:FLEET_WALL_PORT,reason,compatibility,updateRequired:compatibility.updateRequired},account:{accountId:account.accountId,plan:account.plan,mainDeviceId:account.mainDeviceId,fleetProvisioning:account.fleetProvisioning||null},entitlements});
    }
    if (req.method === 'POST' && url.pathname === '/v1/device-channel/fleet-authority') {
      ...
      if(account.fleetProvisioning?.deviceId===ctx.device.deviceId&&account.fleetProvisioning?.state!=='online')account=accounts.setFleetProvisioning(account.accountId,{deviceId:ctx.device.deviceId,state:'ready',reason:'fleet_authority_issued',moduleVersion:ctx.payload.moduleVersion||null,port:FLEET_WALL_PORT});
      return sendJson(res,200,{ok:true,fleet:{desired:true,port:FLEET_WALL_PORT},account:{accountId:account.accountId,plan:account.plan,mainDeviceId:account.mainDeviceId,fleetProvisioning:account.fleetProvisioning||null},entitlements,authority:issued});
    }
    if (req.method === 'POST' && url.pathname === '/v1/device-channel/fleet-status') {
      ...
      const status=String(ctx.payload.status||'').trim().toLowerCase(),port=Number(ctx.payload.port)||FLEET_WALL_PORT;if(status!=='online')throw new FleetAuthorityError('invalid_fleet_status');if(port!==FLEET_WALL_PORT)throw new FleetAuthorityError('invalid_fleet_status_port');
