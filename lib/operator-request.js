const MAX_GET_PAYLOAD_CHARS = 12000;
...
function normalizeShellId(value){
  ...
  if(!['default','bash','zsh','sh','powershell','cmd'].includes(shell))throw httpError('invalid_shell');
  return shell;
}
...
function normalizeSessionOpenPayload(d) {
  ...
  const rawGrace=d.graceMs??d.leaseMs, graceMs=rawGrace==null?undefined:Number(rawGrace);
  if (graceMs!==undefined && !Number.isFinite(graceMs)) throw httpError('invalid_session_grace');
  const rawPreset=d.gracePreset??d.leasePreset, gracePreset=rawPreset==null?undefined:String(rawPreset).trim().toLowerCase();
  if (gracePreset!==undefined && !/^[a-z0-9_-]{1,32}$/.test(gracePreset)) throw httpError('invalid_session_grace_preset');
  return { openId, agentId:aid(d.agentId), label:String(d.label||''), workspace:String(d.workspace||''), graceMs, gracePreset, nodeId:d.nodeId==null?undefined:validId(d.nodeId,/^[A-Za-z0-9._:-]{1,128}$/,'invalid_node_id') };
}
...
function normalizeDeviceHeartbeat(d) {
  ...
  const supportedCapabilities=Array.isArray(d.supportedCapabilities)?cleanCapabilityList(d.supportedCapabilities,{required:true}):undefined;
  return { deviceId:validId(d.deviceId,/^dev_[a-f0-9]{24}$/,'invalid_device_id'), timestamp, nonce, signature, capabilities:cleanCapabilityList(d.capabilities), ...(supportedCapabilities?{supportedCapabilities}:{}), policyRevision:Math.max(0,Number(d.policyRevision)||0) };
}
