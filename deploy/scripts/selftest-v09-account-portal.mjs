...
need(login.includes('/api/auth?action=login')&&register.includes('/api/auth?action=register')&&register.includes('Current Wall password')&&register.includes('ownerPassword'),'account-portal-auth-actions');
...
need(auth.includes("action==='register'")&&auth.includes('ownerPassword')&&auth.includes("action==='device-revoke'")&&auth.includes("action==='devices-revoke-all'"),'account-auth-router');
const gateway=read('gateway/server.mjs');need(gateway.includes('owner_migration_proof_required')&&gateway.includes('wallAuth.verifyCredentials'),'account-owner-migration-proof');
