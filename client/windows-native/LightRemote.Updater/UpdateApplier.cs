...
internal static class UpdateApplier
{
    ...
    {
        WriteStatus("rolling_back",currentVersion,targetVersion,code);var rollback=RecoveryPaths.ResolveRollbackInstaller(currentVersion);if(rollback is null){Log($"rollback_missing preferred={RecoveryPaths.RollbackInstaller(currentVersion)} legacy={RecoveryPaths.LegacyRollbackInstaller(currentVersion)}");WriteReport("failed","LRU140","rollback",currentVersion,targetVersion,currentVersion,true,false,"rollback installer missing");ClearTransaction();TryRestartBackgroundAgentTask();return 20;}
        Log($"rollback_source path={rollback}");await StopInstalledRuntimeAsync(installDir);if(!await TryInstallAndVerifyAsync(rollback,installDir)){Log("rollback_failed");WriteStatus("failed",currentVersion,targetVersion,"LRU141");WriteReport("failed","LRU141","rollback",currentVersion,targetVersion,currentVersion,true,false,"rollback install or health preflight failed");ClearTransaction();TryRestartBackgroundAgentTask();return 21;}
    ...
    {
        await EndScheduledTaskAsync("LightRemoteDeviceAgent");var targets=new[]{Path.GetFullPath(Path.Combine(installDir,"LightRemote.Client.exe")),Path.GetFullPath(Path.Combine(installDir,"GptOperator.Client.exe")),Path.GetFullPath(Path.Combine(installDir,"runtime","node.exe"))};
    ...
    private static async Task<bool> TryInstallAndVerifyAsync(string installer,string installDir)
    {
        ...
        var installedExe=ResolveInstalledClient(installDir);if(installedExe is null)return false;var result=Path.Combine(RecoveryPaths.CacheDir,$"health-{Guid.NewGuid():N}.json");
    ...
    private static string? ResolveInstalledClient(string installDir){foreach(var name in new[]{"LightRemote.Client.exe","GptOperator.Client.exe"}){var exe=Path.Combine(installDir,name);if(File.Exists(exe))return exe;}return null;}
    private static void StartInstalledClient(string installDir){TryRestartBackgroundAgentTask();var exe=ResolveInstalledClient(installDir);if(exe is null)return;try{Process.Start(new ProcessStartInfo(exe,"--background"){UseShellExecute=true});}catch{}}
