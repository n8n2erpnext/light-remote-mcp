...
The v0.6 candidate now runs on ARM and Vercel production-test: persistent local device registry, device/account/node identity propagation, independent heartbeat/TTL presence, named `30m` / `1h` / `3h` plus custom per-session leases, read-only device/session state in executor/gateway/Vercel/Wall, body-safe structured POST operator transport, no static shared bridge Bearer, and a short-lived ARM-validated bridge-session gate. Live production proof confirms anonymous fail-closed, authenticated capabilities/device reads, a 1-hour lease and clean close. `main` and the candidate branch converge on this checkpoint; it is not yet a stable v0.6.0 tag and still does not claim remote-node transport. See `DEVICE_PRESENCE_V0_6.md`.
