...
internal static class Program
{
    ...
    private static void WriteStatus(string state,string currentVersion,string? targetVersion=null,string? code=null)=>WriteJson(RecoveryPaths.StatusFile,new JsonObject{{"state",state},{"currentVersion",currentVersion},{"targetVersion",targetVersion},{"helperVersion",typeof(Program).Assembly.GetName().Version?.ToString()},{"code",code},{"updatedAt",DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()}});
    ...
    private static bool SelfTest(string output,string installDir){try{var process=Environment.ProcessPath??"";var appExe=Path.Combine(installDir,"GptOperator.Client.exe");var ok=File.Exists(process)&&File.Exists(RecoveryPaths.UpdatePublicKey)&&Path.IsPathFullyQualified(installDir),outside=!IsUnder(process,installDir);var result=new JsonObject{{"ok",ok},{"runtime","independent-updater"},{"updater",process},{"app",appExe},{"updaterOutsideApp",outside}};File.WriteAllText(output,result.ToJsonString());return ok&&outside;}catch(Exception ex){File.WriteAllText(output,new JsonObject{{"ok",false},{"error",ex.Message}}.ToJsonString());return false;}}
