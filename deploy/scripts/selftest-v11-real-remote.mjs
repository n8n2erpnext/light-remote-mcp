      ...
      : request.op==='frame'
        ...
        : request.op==='input'
          ? request.args.semanticSessionId
            ? {protocolVersion:1,helperPid:process.pid,appliedEvents:request.args.events.length,sentInputs:2,inputSeq:1,semanticSessionId:request.args.semanticSessionId,epoch:'ep_fake',afterSeq:request.args.afterSeq??2,stateSeq:3,startedAt:10,ackAt:12,elapsedMs:2,cursor:{x:120,y:80},focused:{id:'uia_2',role:'Button',name:'Login'},gap:false,resyncRecommended:false,observation:'local-diff+journal',snapshotNodeCount:2,snapshotTruncated:false,patch:{baseStateSeq:2,stateSeq:3,changed:true,addedCount:0,updatedCount:1,removedCount:0,added:[],updated:[{id:'uia_2',parentId:'uia_1',role:'Button',name:'Continue'}],removed:[]},eventCount:1,eventsCompacted:false,hasMore:false,nextObservation:{mode:'events',afterSeq:3,reason:'patch-applied'},events:[{seq:3,kind:'focus',element:{id:'uia_2',role:'Button',name:'Continue'}}]}
...
try{
  ...
  assert.equal(semanticAckInput.observation,'local-diff+journal');
  assert.equal(semanticAckInput.patch.changed,true);
  assert.equal(semanticAckInput.patch.updated[0].name,'Continue');
  assert.equal(semanticAckInput.eventCount,1);
  assert.equal(semanticAckInput.eventsCompacted,false);
  assert.deepEqual(semanticAckInput.nextObservation,{mode:'events',afterSeq:3,reason:'patch-applied'});
...
const desktopSessionHelper=read('client/windows-native/GptOperator.Client/RealRemoteDesktopSession.cs');
...
for(const token of ['"attach" => DesktopAttach(args)','"resume" => DesktopResume(args)','"detach" => DesktopDetach(args)'])assert.ok(helper.includes(token),`Windows visual desktop opcode missing: ${token}`);
...
for(const token of ['AutomationElement','TreeWalker.ControlViewWalker','semanticSessionId','epoch','stateSeq','password','MaxNodes','nextObservation','snapshot-ready','SnapshotJson','SnapshotStateSeq','SemanticCapture','CaptureSemanticState','UpdateSemanticSnapshotCacheLocked','addedCount','updatedCount','removedCount'])assert.ok(semanticHelper.includes(token),`Windows semantic contract missing: ${token}`);
...
for(const token of ['inputMapping = new','coordinateSpace = "screen-local"','xScale = inputScaleX','yScale = inputScaleY','rounding = "nearest"'])assert.ok(helper.includes(token),'Windows frame/input mapping contract missing: '+token);
...
for(const token of ['SemanticInputContext','InputSeq','afterSeq','settleMs','focusOutsideScope','resyncRecommended','StartedAt','startedAt','ackAt','elapsedMs','nextObservation','CaptureSemanticState','local-diff+journal','patch-applied','eventCount','eventsCompacted'])assert.ok(inputAckHelper.includes(token),`Windows closed-loop input ACK missing: ${token}`);
assert.ok(inputAckHelper.includes('var capture = CaptureSemanticState(session)')&&semanticHelper.includes('RefreshSemanticForegroundRoot(session)'),'Windows UIA input ACK must refresh foreground semantic root through the local capture path after OS input settles');
...
assert.ok(uiaScreenCoordinatesAcceptance.includes('windows-real-remote-desktop-session-attach=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-desktop-session-resume=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-desktop-session-frame-seq=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-desktop-session-throttle=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-desktop-session-unchanged=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-desktop-session-detach=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-desktop-session-detach-negative=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-desktop-session-lease=PASS')&&uiaScreenCoordinatesAcceptance.includes('desktop_session_limit')&&uiaScreenCoordinatesAcceptance.includes('desktop_session_expired')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-screen-topology=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-frame-input-map=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-screen-local-click=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-screen-local-drag=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-screen-topology-pin=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-screen-local-negative=PASS')&&uiaScreenCoordinatesAcceptance.includes('windows-real-remote-screen-local-closed-loop=PASS'),'Screen-local acceptance must prove topology translation, DPI/topology pinning, click, drag and rejection paths');
...
assert.ok(agent.includes("minIntervalMs:Math.max(0,Math.min(Number.isFinite(Number(request.minIntervalMs))?Math.floor(Number(request.minIntervalMs)):250,5000))")&&agent.includes("omitUnchanged:request.omitUnchanged!==false")&&agent.includes("idleTimeoutMs:Math.max(250,Math.min(Number.isFinite(Number(request.idleTimeoutMs))?Math.floor(Number(request.idleTimeoutMs)):120000,900000))"));
...
assert.ok(agent.includes("NATIVE_DESKTOP.request('observe'")&&agent.includes("NATIVE_DESKTOP.request('act'")&&agent.includes("op==='act'")&&agent.includes("local capability denied: desktop-input"));
...
const semanticActions=read('client/windows-native/GptOperator.Client/RealRemoteSemanticActions.cs');
const semanticCore=read('client/windows-native/GptOperator.Client/RealRemoteSemantic.cs');
const semanticEvents=read('client/windows-native/GptOperator.Client/RealRemoteSemanticEvents.cs');
...
const trayRemote=read('client/windows-native/GptOperator.Client/TrayApplicationContext.RealRemote.cs');
const windowsCapabilityRefreshAcceptance=read('deploy/scripts/selftest-v11-capability-refresh-windows.mjs');
const windowsWallAcceptance=read('deploy/scripts/selftest-v11-local-wall-desktop-windows.mjs');
const windowsSemanticActAcceptance=read('client/windows-native/acceptance/real-remote-uia-semantic-act.ps1');
...
for(const source of [read('client/core-files.json'),read('device-agent/install-windows-service.ps1'),read('device-agent/install-linux-service.sh')])assert.ok(!source.includes('local-wall-desktop-page.mjs'),'Legacy Local Wall desktop viewer must not be packaged');
for(const token of ['REAL_REMOTE_ACTIVITY_FILE','recordRealRemoteActivity','realRemoteActivityView','lastViewAt','lastControlAt',"desktopOp==='input'||desktopOp==='act'"])assert.ok(agent.includes(token),'RM local activity/policy contract missing: '+token);
assert.ok(tray.includes('partial class TrayApplicationContext')&&tray.includes('ApplyRealRemoteState(root)')&&tray.includes('RemoteTooltipSuffix()'),'Tray must consume RM activity from local status without a second polling loop');
for(const token of ['Real Remote · Agent viewing','Real Remote · Agent controlling','ShowBalloonTip','An authorized Agent is viewing this device.','An authorized Agent is controlling this device.','Real Remote permissions'])assert.ok(trayRemote.includes(token),'Tray RM indicator contract missing: '+token);
for(const token of ['runtimeSupportedCapabilities','localDeniedCapabilitiesForState','syncRuntimeCapabilityPolicy','supportedCapabilities'])assert.ok(agent.includes(token),'Runtime capability refresh contract missing: '+token);
for(const token of ['remoteEffectiveCapabilitiesForState','serverApproved=new Set','capabilities:remoteEffectiveCapabilitiesForState(state)'])assert.ok(agent.includes(token),'Remote capability intersection contract missing: '+token);
for(const token of ['Device-local permissions.','Authorization identifies this device','supportedCapabilities','New runtime capability'])assert.ok(wall.includes(token),'Runtime capability Permissions UI missing: '+token);assert.ok(!wall.includes('Re-authorize device')&&!wall.includes('pollCapabilityUpgrade'),'Permissions must not trigger device reauthorization for runtime capability changes');
for(const token of ['terminal','desktop','desktop-input','capability_refresh_device_identity_changed','windows-capability-refresh-same-identity=PASS','windows-capability-refresh-default-off=PASS','windows-capability-refresh-stale-pending-cleared=PASS'])assert.ok(windowsCapabilityRefreshAcceptance.includes(token),'Windows capability refresh acceptance missing: '+token);
for(const token of ['capabilityPermissionModelVersion:2','certificateBaseline','clearedPendingEnrollment'])assert.ok(agent.includes(token),'Runtime capability migration contract missing: '+token);
assert.ok(wall.includes('connectCloud.disabled=needsLink;')&&!wall.includes('connectCloud.disabled=needsLink||pendingEnroll'),'Stale pending enrollment must not block Connect for an already-authorized device');
assert.ok(wall.includes('if(left<=0&&!pairRefreshInFlight)refreshPairing(true);'),'Expired A codes must auto-rotate instead of reusing an expired challenge');
for(const token of ['function scheduleStatusRefresh','statusRefreshPromise','statusRefreshQueued','scheduleStatusRefresh();rememberEvent(e)'])assert.ok(wall.includes(token),'Local Wall status refresh coalescing missing: '+token);
for(const token of ['remoteStatusCache','refreshRemoteStatus','loadActivity(limit=200)','loadActivity(200)','setInterval(activityPump,2000)'])assert.ok(wall.includes(token),'Local Wall local-first boot contract missing: '+token);
for(const token of ['setLocalPermissionsForState','allowedCapabilities:[...allowed]','setLocalPermissionsForState(state','await heartbeat(state,base'])assert.ok(agent.includes(token),'Shared daemon permission persistence contract missing: '+token);
assert.ok(agent.includes('OPERATOR_AGENT_CHANNEL_WAIT_MS)||2500'),'Outbound channel default wait must stay bounded at 2500ms for interactive responsiveness');
assert.ok(windowsWorkflow.includes('- name: Capability refresh without reauthorization acceptance')&&windowsWorkflow.includes('selftest-v11-capability-refresh-windows.mjs $exe')&&!windowsWorkflow.slice(windowsWorkflow.indexOf('- name: Capability refresh without reauthorization acceptance'),windowsWorkflow.indexOf('- name: Local Wall RM permissions-only acceptance')).includes('continue-on-error'),'Windows capability refresh acceptance must be a hard gate');
assert.ok(windowsWorkflow.includes("- 'deploy/scripts/selftest-v11-*.mjs'"),'Windows V11 selftest changes must trigger the Windows workflow');
for(const token of ['startLocalWall','Enable Real Remote','Allow Remote control',"req('GET','/desktop')",'windows-local-wall-rm-permissions=PASS','windows-local-wall-rm-viewer-removed=PASS'])assert.ok(windowsWallAcceptance.includes(token),'Windows Local Wall RM permissions acceptance missing: '+token);
assert.ok(windowsWorkflow.includes('- name: Local Wall RM permissions-only acceptance')&&windowsWorkflow.includes('selftest-v11-local-wall-desktop-windows.mjs $exe')&&!windowsWorkflow.slice(windowsWorkflow.indexOf('- name: Local Wall RM permissions-only acceptance'),windowsWorkflow.indexOf('- name: Real Windows OS input acceptance')).includes('continue-on-error'),'Windows Local Wall RM permissions acceptance must be a hard gate');
for(const token of ["'observe'","'act'","nodeId","uia.invoke","sendinput.click","sentInputs","windows-real-remote-semantic-observe-act=PASS"])assert.ok(windowsSemanticActAcceptance.includes(token),'Windows semantic observe-act acceptance missing: '+token);
assert.ok(windowsWorkflow.includes('- name: Real Windows semantic observe-act acceptance')&&windowsWorkflow.includes('real-remote-uia-semantic-act.ps1 -ClientExe $exe')&&!windowsWorkflow.slice(windowsWorkflow.indexOf('- name: Real Windows semantic observe-act acceptance'),windowsWorkflow.indexOf('- name: Real Windows OS input acceptance')).includes('continue-on-error'),'Windows semantic observe-act acceptance must be a hard gate');
...
console.log('v11-real-remote-local-wall-permissions-only=PASS');
console.log('v11-real-remote-capability-refresh-no-reauthorize=PASS');
console.log('v11-real-remote-local-wall-rm-permissions-acceptance=PASS');
