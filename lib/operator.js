...
async function callOperator(path, { method = 'GET', body = null, timeoutMs = 9000, bridgeSession = '', plusSession = '', plusClient = '', accountSession = '', traceId = '' } = {}) {
  ...
  if (plusClient) { headers['x-light-client'] = plusClient; headers.cookie = 'light_remote_client=' + encodeURIComponent(plusClient); }
  if (traceId) headers['x-light-trace'] = String(traceId).slice(0,80);
