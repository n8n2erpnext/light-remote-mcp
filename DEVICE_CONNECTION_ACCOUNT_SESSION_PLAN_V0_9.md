...
**Policy model lock:** Light Remote is a direct remote-computer operator intended to approach RDC capability, not a permanently reduced sandbox. The device advertises every capability it can actually support. The owner-facing Wall policy is a switchboard that may allow or deny any of those capabilities. Turning off `sudo-on-demand` blocks sudo-required work; turning it on permits explicit sudo workflows, while the Light Remote service itself still runs as its normal service user rather than as a permanently-root process. A future Server Safety Policy is a separate platform guardrail for abuse/bypass/deceptive requests and must not be conflated with the owner's Wall switches. Effective execution is the intersection of device-supported capability, owner policy and server safety policy. There must be no hidden product hard-ban that silently removes otherwise-supported remote capability after the owner has explicitly allowed it.
...
All users may see their account-owned device inventory and manage the owner-controlled permission ceiling for each device from a per-device Permissions page. That hosted page and the Local Wall are owner-control surfaces, not the future Server Safety Policy. The UI must expose only capabilities the device declares grantable; Local Wall may impose an immediate device-side deny overlay. VIP adds only a convenience fleet Wall that aggregates several devices into one management screen. VIP does not merge device identities, leases, grants, permissions or sessions into one security context; each device remains isolated underneath the aggregate UI.
## 9. Server-load rule
