...
export class FleetComponentManager{
  ...
  _validatePackage(dir,version){
    ...
    for(const rel of ['device-agent/fleet-wall-runtime.mjs','device-agent/local-wall-auth.mjs','lib/device-proof.mjs','gateway/dashboard.mjs','gateway/device-policy-page.mjs','gateway/brand.mjs','gateway/brand-mark.svg'])if(!fs.existsSync(path.join(dir,rel)))fail(`fleet_component_file_missing:${rel}`);
    return m;
  }
  _prepareRoot(){
