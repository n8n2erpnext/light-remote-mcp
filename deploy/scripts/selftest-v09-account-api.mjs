...
try{
  ...
  const redeemed=await request('POST','/v1/accounts/redeem-license',{key:issued.key},token);
  if(redeemed.status!==200||redeemed.json.account?.plan!=='pro'||redeemed.json.account?.entitlement?.source!=='redeem_key'||!redeemed.json.account?.entitlement?.validUntil)throw new Error('license_redeem_failed');
  const usage=await request('GET','/v1/accounts/usage?months=6',null,token);
  if(usage.status!==200||usage.json.account?.accountId!=='self-hosted-local'||usage.json.entitlements?.plan!=='pro'||!Array.isArray(usage.json.usage?.series)||usage.json.usage.series.length!==6)throw new Error('account_usage_failed');
  ...
  const licenseText=fs.readFileSync(path.join(state,'license-keys.json'),'utf8');
  if(stateText.includes(secret)||stateText.includes(token)||stateText.includes(login.json.token)||licenseText.includes(issued.key))throw new Error('raw_secret_persisted');
  ...
  console.log(JSON.stringify({ok:true,register:true,login:true,me:true,devices:true,licenseRedeem:true,usage:true,logout:true,rawSecretsPersisted:false,emailAuditLeak:false},null,2));
